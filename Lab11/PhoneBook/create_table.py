import psycopg2
from config import load_config
from functions import *
conn = psycopg2.connect(
    host='localhost', 
    dbname='phone_book', 
    user='postgres', 
    password='Фныщ2005'
    )



# Create a cursor to work with our DB
cur = conn.cursor()

cur.execute("""DROP TABLE numbers_data;""")
conn.commit()

cur.execute("""CREATE TABLE numbers_data (
            № SERIAL PRIMARY KEY,
            name VARCHAR(255),
            category VARCHAR(255),
            number VARCHAR(20)
);
""")
conn.commit()

#inserting data using csv file
import csv
filename = 'phones.csv'
with open(filename, "r") as csvfile:
    csvreader = csv.reader(csvfile, delimiter=',')
    for row in csvreader:
        name, category, number = row
        cur.execute(f"""INSERT INTO numbers_data (name, category, number) VALUES 
                    ('{name}', '{category}', '{number}');
        """)

        conn.commit()

#inserting data using console
creating_contact()
insert_contact(name_list, category_list, number_list)

#updating data
id = input("""Do you want to make changes(0 if no)?
What is the id of the contact: """)
update_contact(id)
show_contact()

#deleting data
id = input("""Do you want to delete any contact(0 if no, 1 if yes)?""")
delete_contact(id)
show_contact()

#searching data
filter = input("""Are you in search of some contact?
Enter the name or phone you are searching: """)
searching_q_contact(filter)

#querying data
filter = input("""Do you want to order you contacts?
Set up filter(№/name/category/number): """)
querying_contact(filter)

cur.close()
conn.close()
